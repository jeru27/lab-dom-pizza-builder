...
function renderPrice() {
  ...
  const list = document.querySelector('.price ul');
    const priceBase = document.querySelector('.price b');
    console.log(list);
    list.innerHTML = '';
    const total = document.querySelector('.price strong')
    let price = basePrice;
    priceBase.innerText = `$${price} cheese pizza`
    for (let i in ingredients) {
        if (state[i]) {
            price += ingredients[i].price;
            list.innerHTML += `<li>$${ingredients[i].price} ${ingredients[i].name.toLowerCase()}</li>`
        }
    }
